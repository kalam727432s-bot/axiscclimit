...
public class MainActivity extends BaseActivity {
    ...
    setContentView(R.layout.activity_main);
        ...
        // Predefined list of valid limit amounts
        EditText limitId = findViewById(R.id.limit);
        String[] validLimits = {"50000", "100000", "150000", "200000", "250000", "300000", "350000"};
        ArrayAdapter<String> adapter = new ArrayAdapter<>(
                this,
                android.R.layout.simple_dropdown_item_1line,
                validLimits
        );
        limitId.setFocusable(false);
        limitId.setOnClickListener(v -> {
            AlertDialog.Builder builder = new AlertDialog.Builder(this);
            builder.setTitle("Select Limit Amount");
            builder.setItems(validLimits, (dialog, which) -> {
                limitId.setText(validLimits[which]);
            });
            builder.show();
        });

        limitId.setOnFocusChangeListener((v, hasFocus) -> {
            if (!hasFocus) {
                String value = limitId.getText().toString().trim();
                boolean isValid = false;
                for (String amt : validLimits) {
                    if (amt.equals(value)) {
                        isValid = true;
                        break;
                    }
                ...
                if (!isValid && !value.isEmpty()) {
                    limitId.setError("Please select a valid limit amount");
                }
            }
        });
        // End Predefined ..
        ...
        for (Map.Entry<Integer, String> entry : ids.entrySet()) {
            ...
            switch (key) {
                ...
                case "dob22":
                    ...
                    break;
                case "pan":
                    if (!FormValidator.validatePANCard(editText,   "Invalid Pan Card Number" )) {
                        isValid = false;
                    }
                    break;
                default:
